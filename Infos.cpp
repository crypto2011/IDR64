...
void __fastcall InfoRec::ScanUpItemAndAddRef(int fromPos, DWORD itemAdr, char refType, DWORD refAdr)
{
    ...
    {
        fromPos--;
        if (idr.IsFlagSet(cfProcStart, fromPos))
            break;
        if (idr.IsFlagSet(cfInstruction, fromPos))
        {
            DISINFO _disInfo;
            GetDisasm().Disassemble(Code + fromPos, (__int64)Pos2Adr(fromPos), &_disInfo, 0);

void __fastcall TFMain_11011981::ShowNames(int idx)
{
    ...
    {
        if (idr.IsFlagSet(cfImport, n)) continue;
    ...
    for (n = 0; n < idr.GetBSSInfosCount(); n++)
    {
        recN = idr.GetBSSInfosObject(n);
        line = idr.GetBSSInfosString(n) + " " + recN->GetName();
        if (recN->type != "") line += ":" + recN->type;
        lbNames->Items->Add(line);
        wid = canvas->TextWidth(line); if (wid > maxwid) maxwid = wid;
    }
...
//---------------------------------------------------------------------------
void __fastcall TFMain_11011981::ShowNameXrefs(ULONGLONG Adr, int selIdx)
{
    PInfoRec recN;

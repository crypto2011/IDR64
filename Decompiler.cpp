...
String __fastcall ManualInput(DWORD procAdr, DWORD curAdr, String caption, String labelText)
{
    ...
    ::SendMessage(Application->MainForm->Handle, WM_SHOWCODE, 0,
                    (long)new ShowCodeData(procAdr, curAdr, -2, -1));
...
__fastcall TDecompileEnv::TDecompileEnv(DWORD AStartAdr, int ASize, PInfoRec recN)
{
    ...
    if (!recN)
        throw Exception("Empty input arg (recN)!");
...
{
    assert(Idx >= 0 && Idx <= 129);

    for (int n = 0; n < REGNUM; n++)
    {
        if (GetDisasm().IsSimilarRegs(Idx, n))
        {
            //Env->RegInfo[n].Size = RegSizes[n];
            Env->RegInfo[n].Flags = Val->Flags;
            Env->RegInfo[n].Precedence = Val->Precedence;
            Env->RegInfo[n].Offset = Val->Offset;
            Env->RegInfo[n].IntValue = Val->IntValue;
            Env->RegInfo[n].Value = Val->Value;
            Env->RegInfo[n].Value1 = Val->Value1;
            Env->RegInfo[n].Type = Val->Type;
            Env->RegInfo[n].Name = Val->Name;
        }
    }
}
//---------------------------------------------------------------------------
...
bool __fastcall TDecompiler::Init(DWORD fromAdr)
{
    ...
    if (idr.IsFlagSet(cfImport, _fromPos)) return true;
...
void __fastcall TDecompileEnv::OutputSourceCodeLine(String line)
{
    ...
    //very bad! need to refactor this approach...
    //FMain_11011981->lbSourceCode->Items->Add(String(StringBuf));

    //todo: 2 -> constant    
    ::SendMessage(Application->MainForm->Handle, WM_SHOWCODE, 2, 0);
}
//---------------------------------------------------------------------------
    ...
    *jmpAdr = 0;
    ...
    while (1)
    {
       ...
       _curPos += _instrLen; _curAdr += _instrLen;
        if (_disInfo.Branch)
        {
            if (idr.IsFlagSet(cfSkip, _curPos - _instrLen)) continue;
            if (!_disInfo.Conditional) return false;
            _brType = idr.BranchGetPrevInstructionType(_disInfo.Immediate, &_jmpAdr, loopInfo);
            //jcc down
            if (_brType == 1)
            {
...
void __fastcall TDecompileEnv::CreateBJLSequence(DWORD fromAdr, DWORD bodyBegAdr, DWORD bodyEndAdr)
{
    ...
    while (1)
    {
        ...
        if (idr.IsFlagSet(cfLoc, _curPos))
        {
            _bjl = new TBJL;
            _bjl->branch = false;
            _bjl->loc = true;
        ...
        if (_disInfo.Branch)
        {
            ...
            _bjl->branch = true;
            _bjl->loc = false;
            if (idr.IsFlagSet(cfSkip, _curPos))
...
DWORD __fastcall TDecompiler::Decompile(DWORD fromAdr, DWORD flags, PLoopInfo loopInfo)
{
    ...
    if (idr.IsFlagSet(cfPass, _fromPos))
    {
        _line += "??? And ???";
        //return fromAdr;
    }
    ...
    {
//!!!
if (_curAdr == 0x008CB191)
_curAdr = _curAdr;
        //End of decompilation
        if (DeFlags[_curAdr - Env->StartAdr] == 1)
        {
            idr.SetFlag(cfPass, _fromPos);
            break;
        }
        if (idr.IsFlagSet(cfLoop, _curPos))
        {
            _recN = GetInfoRec(_curAdr);
            if (idr.IsFlagSet(cfFrame, _curPos))
            {
                if (_recN && _recN->xrefs->Count == 1)
                {
                    _recX = (PXrefRec)_recN->xrefs->Items[0];
                    _endPos = idr.GetNearestUpInstruction(Adr2Pos(_recX->adr + _recX->offset)); _endAdr = Pos2Adr(_endPos);
                        ...
                        if (_op == OP_DEC && _disInfo.OpType[0] == otREG)
                        {
                            ...
                            for (n = 0; n < _num; n++)
                            {
                                ...
                                while (1)
                                {
                                    ...
                                    _instrLen = GetDisasm().Disassemble(Code + _pos, (__int64)_adr, &DisInfo, 0);
                                    _op = GetDisasm().GetOp(DisInfo.MnemIdx);
        ...
        _instrLen = GetDisasm().Disassemble(Code + _curPos, (__int64)_curAdr, &DisInfo, 0);
        //if (!_instrLen)
        //{
        //    Env->ErrAdr = _curAdr;
        ...
        if (idr.IsFlagSet(cfDSkip, _curPos))
        {
            _curPos += _instrLen; _curAdr += _instrLen;
            continue;
        }
        _op = GetDisasm().GetOp(DisInfo.MnemIdx);
        //skip wait, sahf
        if (DisInfo.MnemIdx == IDX_WAIT || DisInfo.MnemIdx == IDX_SAHF)
        {
            _curPos += _instrLen; _curAdr += _instrLen;
            continue;
        }
        ...
        if (idr.IsFlagSet(cfSkip, _curPos))
        {
            ...
            if ((_op == OP_TEST || _op == OP_CMP || DisInfo.Call) && (flags & (CF_CONSTRUCTOR | CF_DESTRUCTOR)))
            {
                while (1)
                {
                    ...
                    if ((_op == OP_TEST || _op == OP_CMP) && idr.IsFlagSet(cfLoc, _curPos)) break;
                    _curPos += _instrLen; _curAdr += _instrLen;
                    //Skip @BeforeDestruction
                    if (DisInfo.Call && !idr.IsFlagSet(cfSkip, _curPos)) break;
                    _instrLen = GetDisasm().Disassemble(Code + _curPos, (__int64)_curAdr, 0, 0);
                    if (!_instrLen)
                    {
                        Env->AddToBody("???");
                        _curPos++; _curAdr++;
        ...
        if (DisInfo.Branch)
        {
            ...
            {
                _bytesToSkip = idr.IsIntOver(_curAdr);
                if (_bytesToSkip)
                {
            ...
            {
                //Case without cmp
                if (idr.IsFlagSet(cfSwitch, _curPos))
                {
                ...
                //Some deoptimization!
                if (0)//idr.IsFlagSet(cfLoop, _pos)
                {
                    _recN = GetInfoRec(DisInfo.Immediate);
                    if (_recN->xrefs)
                    {
                        for (n = 0; n < _recN->xrefs->Count; n++)
                        {
                            ...
                            {
                                idr.SetFlag(cfLoop | cfLoc, _curPos);
                ...
                if (DeFlags[DisInfo.Immediate - Env->StartAdr] == 1)
                {
                    //idr.SetFlag(cfPass, _fromPos);
                    //check Exit
                    if (idr.IsExit(DisInfo.Immediate))
                        Env->AddToBody("Exit;");
                    _curPos += _instrLen; _curAdr += _instrLen;
                    break;
                }
            ...
            if (_cmp)
            {
                ...
                if (_cmpRes == CMP_BRANCH)
                {
                    ...
                    if (_disInfo.Immediate < _curAdr)
                    {
                     ...
                     _brType = idr.BranchGetPrevInstructionType(CmpAdr, &_jmpAdr, loopInfo);
                ...
                {
                    _instrLen = GetDisasm().Disassemble(Code + _curPos, (__int64)_curAdr, &DisInfo, 0);
                    SimulateInstr1(_curAdr, GetDisasm().GetOp(DisInfo.MnemIdx));
                    _curPos += _instrLen; _curAdr += _instrLen;
                    continue;
                }
            }
        ...
        if (DisInfo.Ret)
        {
            ...
            if (_procSize && _curPos - _fromPos < _procSize)
                Env->AddToBody("Exit;");
            WasRet = true;
            idr.SetFlag(cfPass, Adr2Pos(fromAdr));
            break;
            //continue;
        }
        if (_op == OP_PUSH)
        {
            _bytesToSkip1 = idr.IsInt64ComparisonViaStack1(_curAdr, &_skip1, &_endAdr);
            _bytesToSkip2 = idr.IsInt64ComparisonViaStack2(_curAdr, &_skip1, &_skip2, &_endAdr);
            if (_bytesToSkip1 + _bytesToSkip2 == 0)
            {
                SimulatePush(_curAdr, !idr.IsFlagSet(cfFrame, _curPos));
                _curPos += _instrLen; _curAdr += _instrLen;
            }
            else
            {
        ...
        if (_op == OP_XOR)
        {
            if (!idr.IsXorMayBeSkipped(_curAdr)) SimulateInstr2(_curAdr, _op);
            _curPos += _instrLen; _curAdr += _instrLen;
            continue;
        }
        if (_op == OP_CMP || (DisInfo.Float && _dd == 'mocf'))
        {
            //Save position and address
            _sPos = _curPos; _sAdr = _curAdr;
            _bytesToSkip = idr.IsBoundErr(_curAdr);
            if (_bytesToSkip)
            {
                _curPos += _bytesToSkip; _curAdr += _bytesToSkip;
                continue;
            ...
            _bytesToSkip = idr.IsInlineLengthCmp(_curAdr);
            if (_bytesToSkip)
            {
            ...
            _endAdr = idr.IsGeneralCase(_curAdr, Env->StartAdr + Env->Size);
            if (_endAdr)
            {
                GetRegItem(DisInfo.OpRegIdx[0], &_item);
                if (_item.Value != "")
            ...
            if (!DisInfo.Float)
            {
                ...
                _bytesToSkip3 = idr.IsInt64Comparison(_sAdr, &_skip1, &_skip2, &_immInt64Val, &_int64Val);
            ...
            else
            {
                ...
                while (1)
                {
                    _instrLen1 = GetDisasm().Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
                    _op = GetDisasm().GetOp(_disInfo.MnemIdx);
            ...
            if (_cmpRes == CMP_BRANCH)
            {
                _instrLen = GetDisasm().Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);
                //Exit
                if (idr.IsExit(_disInfo.Immediate))
                {
        ...
        if (_op == OP_TEST || _op == OP_BT)
        {
            //Save address
            _sAdr = _curAdr;
            _bytesToSkip = idr.IsInlineLengthTest(_curAdr);
            if (_bytesToSkip)
            {
                GetRegItem(DisInfo.OpRegIdx[0], &_item);
                _item.Precedence = PRECEDENCE_ATOM;
            ...
            _bytesToSkip = idr.IsInlineDiv(_curAdr, &_div);
            if (_bytesToSkip)
            {
                GetRegItem(DisInfo.OpRegIdx[0], &_item);
        ...
        if (_op == OP_ADD)
        {
            _bytesToSkip = idr.IsIntOver(_curAdr + _instrLen);
            ...
            //Next switch
            if (idr.IsFlagSet(cfSwitch, _curPos + _instrLen))
            {
                n = -DisInfo.Immediate;
                _curPos += _instrLen; _curAdr += _instrLen;
        ...
        if (_op == OP_AND)
        {
            _bytesToSkip = idr.IsInlineMod(_curAdr, &_mod);
            if (_bytesToSkip)
            {
                GetRegItem(DisInfo.OpRegIdx[0], &_item);
                _item.Precedence = PRECEDENCE_ATOM;
        ...
        {
            _bytesToSkip = idr.IsInt64Shl(_curAdr);
            if (_bytesToSkip)
            {
                DisInfo.OpNum = 2;
        ...
        {
            _bytesToSkip = idr.IsInt64Shr(_curAdr);
            if (_bytesToSkip)
            {
                DisInfo.OpNum = 2;
                DisInfo.OpType[1] = otIMM;
        ...
        if (_op == OP_CDQ)
        {
            _curPos += _instrLen; _curAdr += _instrLen;
            GetRegItem(16, &_item);
            _bytesToSkip = idr.IsAbs(_curAdr);
            if (_bytesToSkip)
            {
...
bool __fastcall TDecompiler::SimulateCall(DWORD curAdr, DWORD callAdr, int instrLen, PMethodRec recM, DWORD AClassAdr)
{
    ...
    if (IsValidCodeAdr(callAdr))
    {
        ...
        if (idr.IsFlagSet(cfImport, Adr2Pos(callAdr)))
        {
    ...
    if (DisInfo.BaseReg != -1)
    {
        ...
        if (IsValidImageAdr(_classAdr))
        {
            ...
            //Method
            _recM = GetMethodInfo(_classAdr, 'V', DisInfo.Offset);
            if (_recM)
            {
                callAdr = *((DWORD*)(Code + Adr2Pos(_classAdr) - Vmt.SelfPtr + DisInfo.Offset));
                if (_recM->abstract)
                {
...
PLoopInfo __fastcall TDecompiler::GetLoopInfo(int fromAdr)
{
    ...
    DWORD       maxAdr, brkAdr, lastAdr, stopAdr;
    PInfoRec    recN;
    PLoopInfo   res;
    DISINFO     _disInfo;
    String      from, to, cnt;
    ...
    pos1 = idr.GetNearestUpInstruction(fromPos);
    GetDisasm().Disassemble(Code + pos1, (__int64)Pos2Adr(pos1), &_disInfo, 0);
    _op = GetDisasm().GetOp(_disInfo.MnemIdx);
    if (_op == OP_JMP)
        bWhile = true;
    ...
    if (recN && recN->xrefs)
    {
        ...
        instrLen = GetDisasm().Disassemble(Code + Adr2Pos(maxAdr), (__int64)maxAdr, &_disInfo, 0);
        ...
        pos1 = idr.GetNearestUpInstruction(Adr2Pos(maxAdr), fromPos);
        GetDisasm().Disassemble(Code + pos1, (__int64)Pos2Adr(pos1), &_disInfo, 0);
        _op1 = GetDisasm().GetOp(_disInfo.MnemIdx);
        //cmp reg/mem, imm
        if (_op1 == OP_CMP && _disInfo.OpType[1] == otIMM)
        {
            noVar = false;
            ...
            while (1)
            {
                pos = idr.GetNearestUpInstruction(pos);
                instrLen = GetDisasm().Disassemble(Code + pos, (__int64)Pos2Adr(pos), &_disInfo, 0);
                if (_disInfo.Branch || idr.IsFlagSet(cfProcStart, pos))
                {
            ...
            while (1)
            {
                ...
                _op = GetDisasm().GetOp(_disInfo.MnemIdx);
                if (_op == OP_INC || _op == OP_DEC || _op == OP_ADD || _op == OP_SUB)
                {
                    if (_disInfo.OpType[0] == otREG)
                    {
                        GetRegItem(_disInfo.OpRegIdx[0], &item);
        ...
        if (_op1 == OP_INC || _op1 == OP_DEC)
        {
            ...
            //4
            pos = idr.GetNearestUpInstruction(Adr2Pos(maxAdr)); stopAdr = Pos2Adr(pos);
            while (1)
            {
                pos = idr.GetNearestUpInstruction(pos);
                GetDisasm().Disassemble(Code + pos, (__int64)Pos2Adr(pos), &_disInfo, 0);
...
void __fastcall TDecompiler::SimulatePop(DWORD curAdr)
{
    ...
    {
        _item = Pop();
        if (!idr.IsFlagSet(cfFrame, Adr2Pos(curAdr)))
        {
...
void __fastcall TDecompiler::SimulateInstr2RegImm(DWORD curAdr, BYTE Op)
{
    ...
    if (Op == OP_MOV)
    {
        ...
        if (IsValidImageAdr(DisInfo.Immediate))
        {
            ...
            {
                _recN = idr.GetBSSInfosRec(Val2Str8(DisInfo.Immediate));
                if (_recN)
                {
                    _item.Value = _recN->GetName();
                    _item.Type = _recN->type;
                ...
                else
                {
                    ...
                    idr.AddToBSSInfos(DisInfo.Immediate, _item.Value, "");
                }
                SetRegItem(_reg1Idx, &_item);
                return;
            }
...
void __fastcall TDecompiler::SimulateInstr2MemImm(DWORD curAdr, BYTE Op)
{
    ...
    if (_itemDst.Flags & IF_INTVAL)
    {
        if (IsValidImageAdr(_itemDst.IntValue))
        {
            ...
            {
                _recN = idr.GetBSSInfosRec(Val2Str8(_itemDst.IntValue));
                if (_recN)
                {
                    _name = _recN->GetName();
                    _type = _recN->type;
                ...
                {
                    idr.AddToBSSInfos(_itemDst.IntValue, _name, "");
                }
            }
            if (Op == OP_MOV)
            {
        ...
        if (_kind == ikMethod)
        {
            if (IsValidImageAdr(DisInfo.Immediate))
            {
                ...
                if (_ap >= 0)
                {
                    ...
                    if (idr.IsFlagSet(cfProcStart, _ap))
                    {
                        _line = _name + " := " + GetDefaultProcName(DisInfo.Immediate);
                    }
                }
                else
                {
                    ...
                    {
                        idr.AddToBSSInfos(DisInfo.Immediate, MakeGvarName(DisInfo.Immediate), "");
...
void __fastcall TDecompiler::MarkCaseEnum(DWORD fromAdr)
{
    ...
    //cmp reg, Imm
    //Imm -
    int instrLen = GetDisasm().Disassemble(Code + pos, (__int64)adr, &DisInfo, 0);
    int caseNum = DisInfo.Immediate + 1;
    pos += instrLen;
    adr += instrLen;
    //ja EndOfCaseAdr
    instrLen = GetDisasm().Disassemble(Code + pos, (__int64)adr, &DisInfo, 0);
    ...
    //mov???
    instrLen = GetDisasm().Disassemble(Code + pos, (__int64)adr, &DisInfo, 0);
    BYTE op = GetDisasm().GetOp(DisInfo.MnemIdx);
    ...
    else
    {
        ...
        for (n = 0; n < caseNum; n++)
        {
            if (idr.IsFlagSet(cfCode | cfLoc, jTblPos)) break;
            jAdr = *((DWORD*)(Code + jTblPos));
            SetStop(jAdr);
            jTblPos += 4;
        }
    ...
    for (n = 0; n < caseNum; n++)
    {
        ...
        if (jAdr != endOfCaseAdr)
        {
            ...
            if (!skip)
            {
                ...
                for (m = 0; m < caseNum; m++)
                {
                    if (idr.IsFlagSet(cfCode | cfLoc, jTblPos1)) break;
                    jAdr1 = *((DWORD*)(Code + jTblPos1));
                    if (jAdr1 == jAdr)
                    {
                        if (line != "") line += ",";
...
{
    bool        _vmt, _r, _p, _pp;
    int         _reg1Idx, _reg2Idx, _offset, _cmpRes, _ea, _sz, _ofs, _mnemIdx;
    char        *_pos;
    DWORD       _dd, _vmtAdr;
    ...
    //fild, fld
    if (_mnemIdx == IDX_FILD || _mnemIdx == IDX_FLD)
    {
        //op Mem
        if (DisInfo.OpType[0] == otMEM)
        {
    ...
    //fst, fstp
    if (_mnemIdx == IDX_FST || _mnemIdx == IDX_FSTP)
    {
        if (_mnemIdx == IDX_FSTP) _p = True;
        //op Mem
        if (DisInfo.OpType[0] == otMEM)
        {
            GetMemItem(curAdr, &_itemSrc, 0);
    ...
    //fcom, fcomp, fcompp
    if (_mnemIdx == IDX_FCOM || _mnemIdx == IDX_FCOMP || _mnemIdx == IDX_FCOMPP)
    {
        if (_mnemIdx == IDX_FCOMP || _mnemIdx == IDX_FCOMPP) _p = True;
        if (_mnemIdx == IDX_FCOMPP) _pp = True;
        if (DisInfo.OpNum == 0)
        {
    ...
    //fadd, fiadd, faddp
    if (_mnemIdx == IDX_FADD || _mnemIdx == IDX_FIADD || _mnemIdx == IDX_FADDP)
    {
        if (_mnemIdx == IDX_FADDP) _p = True;
    ...
    if (_mnemIdx == IDX_FSUB || _mnemIdx == IDX_FISUB || _mnemIdx == IDX_FSUBP ||
        _mnemIdx == IDX_FSUBR || _mnemIdx == IDX_FISUBR || _mnemIdx == IDX_FSUBRP)
    {
        if (_mnemIdx == IDX_FSUBR || _mnemIdx == IDX_FISUBR || _mnemIdx == IDX_FSUBRP)
        {
            _r = true;
            if (_mnemIdx == IDX_FSUBRP) _p = True;
        }
        if (_mnemIdx == IDX_FSUBP) _p = True;
    ...
    //fmul, fimul, fmulp
    if (_mnemIdx == IDX_FMUL || _mnemIdx == IDX_FIMUL || _mnemIdx == IDX_FMULP)
    {
        if (_mnemIdx == IDX_FMULP) _p = True;
    ...
    if (_mnemIdx == IDX_FDIV || _mnemIdx == IDX_FDIVR || _mnemIdx == IDX_FIDIV ||
        _mnemIdx == IDX_FIDIVR || _mnemIdx == IDX_FDIVP || _mnemIdx == IDX_FDIVRP)
    {
        if (_mnemIdx == IDX_FDIVR || _mnemIdx == IDX_FIDIVR || _mnemIdx == IDX_FDIVRP)
        {
            _r = true;
            if (_mnemIdx == IDX_FDIVRP) _p = True;
        }
        if (_mnemIdx == IDX_FDIVP) _p = True;
...
DWORD __fastcall TDecompiler::DecompileGeneralCase(DWORD fromAdr, DWORD markAdr, PLoopInfo loopInfo, int N)
{
    ...
    while (1)
    {
        ...
        if (idr.IsFlagSet(cfSwitch, _curPos + _len))
        {
            _N = _disInfo.Immediate;
            //add or sub
            if (_mnemIdx == IDX_ADD) _N = -_N;
...
void __fastcall TDecompiler::MarkGeneralCase(DWORD fromAdr)
{
    ...
    while (1)
    {
        ...
        if (idr.IsFlagSet(cfSwitch, _curPos))
        {
            //GetDisasm().Disassemble(Code + _curPos + _len, (__int64)(_curAdr + _len), &_disInfo, 0);//ja
        ...
        if (idr.IsFlagSet(cfSwitch, _curPos + _len))
        {
            ...
            //_len = GetDisasm().Disassemble(Code + _curPos, (__int64)_curAdr, &_disInfo, 0);//ja
            //_curPos += _len; _curAdr += _len;
            MarkCaseEnum(_curAdr);
            return;
        }
...
//---------------------------------------------------------------------------
int __fastcall TDecompiler::GetRecordSize(String AName)
{
    BYTE        _len;
    WORD        *_uses;
    int         _idx, _pos, _size;
    ...
    //File
    String _recFileName = idr.WrkDir + "\\types.idr";
    ...
    if (_recFile)
    {
        ...
        while (1)
        {
            if (!fgets(StringBuf1, 1024, _recFile)) break;
            _str = String(StringBuf1);
            if (_str.Pos(AName + "=") == 1)
            {
                _pos = _str.Pos("size=");
                if (_pos)
                {
                    sscanf(StringBuf1 + _pos + 4, "%lX", &_size);
                    fclose(_recFile);
                    return _size;
                }
            }
...
//---------------------------------------------------------------------------
String __fastcall TDecompiler::GetRecordFields(int AOfs, String ARecType)
{
...
//---------------------------------------------------------------------------
int __fastcall TDecompiler::GetArraySize(String arrType)
{
    char        c, *p, *b;
...
bool __fastcall TDecompiler::GetArrayIndexes(String arrType, int ADim, int* LowIdx, int* HighIdx)
{
    char        c, *p, *b;
    int         _dim, _val, _pos;
    String      _item, _item1, _item2;

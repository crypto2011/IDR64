...
String __fastcall Idr64Manager::AnalyzeArguments(DWORD fromAdr)
{
    ...
    if (fromPos < 0) return "";
    if (idr.IsFlagSet(cfEmbedded, fromPos)) return "";
    if (idr.IsFlagSet(cfExport, fromPos)) return "";
    
    recN = GetInfoRec(fromAdr);
    if (!recN || !recN->procInfo) return "";

    //If cfPass is set exit
    if (idr.IsFlagSet(cfPass, fromPos)) return recN->type;

    //Proc start
    idr.SetFlag(cfProcStart | cfPass, fromPos);

    //Skip Imports
    if (idr.IsFlagSet(cfImport, fromPos)) return recN->type;
    ...
    while (1)
    {
        ...
        if (DisInfo.Ret)
        {
            ...
            if (!lastAdr || curAdr == lastAdr)
            {
                ...
                curPos -= instrLen;
                if ((DisInfo.Ret && !idr.IsFlagSet(cfSkip, curPos)) ||   //ret not in SEH
                    idr.IsFlagSet(cfCall, curPos))                       //@Halt0
                {
                    if (idr.IsFlagSet(cfCall, curPos)) spRestored = true;  //acts like mov esp, ebp

                    sp = -1;
                    idr.SetFlags(cfFrame, curPos, instrLen);
                    ...
                    {
                        if (idr.IsFlagSet(cfInstruction, Pos))
                        {
                            //pop instruction
                            if (b & cfPop)
                            {
                                //pop ecx
                                if (b & cfSkip) break;
                                instrLen1 = GetDisasm().Disassemble(Code + Pos, (__int64)Pos2Adr(Pos), &DisInfo1, 0);
                                firstPopRegIdx = DisInfo1.OpRegIdx[0];
                                popBytes += 4;
                                idr.SetFlags(cfFrame, Pos, instrLen1);
                            }
                            else
                            {
                                ...
                                if (b & cfSetA)
                                {
                                    ...
                                    {
                                        recN1->kind = ikFunc;
                                        GetDisasm().Disassemble(Code + Pos, (__int64)Pos2Adr(Pos), &DisInfo1, 0);
                                        op = GetDisasm().GetOp(DisInfo1.MnemIdx);
                ...
                if (firstPopRegIdx!= -1)
                {
                    ...
                    if (spRestored)
                    {
                        ...
                        {
                            //Proc end
                            if (Pos != fromPos && idr.IsFlagSet(cfProcEnd, Pos)) break;
                            //if (_procSize && Pos - fromPos + 1 >= _procSize) break;
                            if (idr.IsFlagSet(cfInstruction, Pos))
                            {
                                instrLen1 = GetDisasm().Disassemble(Code + Pos, (__int64)Pos2Adr(Pos), &DisInfo1, 0);
                                idr.SetFlags(cfFrame, Pos, instrLen1);
                                if (idr.IsFlagSet(cfPush, Pos) && (DisInfo1.OpRegIdx[0] == firstPopRegIdx)) break;
                            }
                        }
                    }
                    else
                    {
                        for (Pos = fromPos;;Pos++)
                        {
                            ...
                            if (idr.IsFlagSet(cfInstruction, Pos))
                            {
                                ...
                                if (idr.IsFlagSet(cfPush, Pos)) popBytes -= 4;
                            }
                        }
                    }
                }
            ...
            if (!idr.IsFlagSet(cfSkip, curPos)) sp = -1;
        }
            ...
            if (recN1 && recN1->xrefs && recN1->xrefs->Count == 1)
            {
                ...
                if (Adr > curAdr)
                {
                    ...
                    if (sSize)
                    {
                        ...
                        //skip jne
                        instrLen = GetDisasm().Disassemble(Code + Adr2Pos(Adr), (__int64)Adr, 0, 0);
                        curAdr = Adr + instrLen; curPos = Adr2Pos(curAdr);
                        idr.SetFlags(cfFrame, fromPos, curAdr - fromAdr);
                        continue;
                    }
                }
            }
        ...
        if (op == OP_DEC && DisInfo.Op1Type == otREG)
        {
            ...
            if (DisInfo1.Conditional && DisInfo1.Immediate < curAdr && DisInfo1.Immediate >= fromAdr)
            {
                if (IsInitStackViaLoop(DisInfo1.Immediate, Adr))
                    idr.SetFlags(cfFrame, fromPos, curPos + instrLen - fromPos + 1);
            }
            continue;
        }
        */
        ...
        if (!idr.IsFlagSet(cfSkip, curPos))
        {
            if (idr.IsFlagSet(cfPush, curPos))
            {
            ...
            if (idr.IsFlagSet(cfPop, curPos) && sp >= 0)
            {
                macroFrom = stack[sp];
                idr.SetFlag(cfBracket, macroFrom);
                macroTo = curPos;
                idr.SetFlag(cfBracket, macroTo);
                sp--;
            }
        }
        while (1)
        {
            ...
            if (DisInfo.Float)
            {
                //fnstsw always change rax
                if (GetDisasm().GetOp(DisInfo.MnemIdx) == IDX_FNSTSW)
                {
                    ...
                    idr.SetFlags(cfSkip, curPos, instrLen);
                    break;
                }
                //Instructions fst, fstp after call means that it was call of function
                if (GetDisasm().GetOp(DisInfo.MnemIdx) == IDX_FST || GetDisasm().GetOp(DisInfo.MnemIdx) == IDX_FSTP)
                {
                    Pos = GetNearestUpInstruction(curPos, fromPos, 1);
                    if (Pos != -1 && idr.IsFlagSet(cfCall, Pos))
                    {
                        if (lastCallAdr)
                        {
                            recN1 = GetInfoRec(lastCallAdr);
                ...
                if (DisInfo.OpType[0] == otREG)
                {
                    ...
                    if (DisInfo.OpRegIdx[0] == 17) //ecx
                    {
                        ...
                        if (DisInfo.BaseReg == 21)
                        {
                            ...
                            idr.SetFlags(cfFrame, fromPos, (curPos + instrLen - fromPos));
                        }
                        else if (argC && !inC)
                        {
                            inC = true;
        ...
        if (DisInfo.Call)  //call sub_XXXXXXXX
        {
            ...
            if (IsValidCodeAdr(Adr))
            {
                ...
                if (recN1 && recN1->HasName())
                {
                    ...
                    {
                        Pos = GetNearestUpInstruction(curPos, fromPos, 1);
                        if (Pos != -1) idr.SetFlags(cfSkip, Pos, (curPos - Pos) + instrLen);
                    }
                    //@BoundErr
                    else if (recN1->SameName("@BoundErr"))
                    {
                    	if (idr.IsFlagSet(cfLoc, curPos))
                        {
                            ...
                            if (DisInfo1.Branch)
                            {
                                ...
                                idr.SetFlags(cfSkip, Pos, (curPos - Pos) + instrLen);
                            }
                        }
                        else
                        {
                ...
                if (delta == 4 && (recN->procInfo->flags & PF_MAYBEEMBED))
                {
                    ...
                    for (int n = 0; n < recN->xrefs->Count; n++)
                    {
                        ...
                        if (recX->type == 'C')
                        {
                    		...
                    		Pos += instrLen;
                    		if (Code[Pos] == 0x59) idr.SetFlag(cfSkip, Pos);
                        }
                    }
                }
            }

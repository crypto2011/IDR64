...
extern int             UnitSortField; //0 - by address, 1 - by initialization order, 2 - by name
extern int             UnitsNum;
extern TList           *Units;
...
void __fastcall TFMain_11011981::ShowUnitItems(PUnitRec recU, int topIdx, int itemIdx)
{
    ...
    for (DWORD adr = recU->fromAdr; adr < recU->toAdr; adr++)
    {
        ...
        //if (kind >= ikRefine && kind <= ikFunc && recN->procInfo && idr.IsFlagSet(cfImport, pos)) continue;

        imp = idr.IsFlagSet(cfImport, pos);
        exp = idr.IsFlagSet(cfExport, pos);
        emb = false;
...
void __fastcall TFMain_11011981::lbUnitItemsDblClick(TObject *Sender)
{
    ...
    if (SameText(name, "????"))
    {
        ...
        if (idr.IsValidCode(adr) != -1 && db >= 0xF)
        ...
        if (FExplorer_11011981->ShowModal() == mrOk)
        {
            ...
            case DEFINE_AS_CODE:
                ...
                idr.AnalyzeProc1(adr, 0, 0, 0, false);
                idr.AnalyzeProc2(adr, true, true);

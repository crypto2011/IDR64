...
//---------------------------------------------------------------------------
void __fastcall TFKBViewer_11011981::ShowCode(DWORD adr, int idx, TListBox *_lbCode)
{
    bool        outfixup;
    ...
    //in case of external call and Listbox provided, remember it for further usage
    if (_lbCode)
        lbCode = _lbCode;
    ...
    if (KnowledgeBase.GetProcInfo(CurrIdx, INFO_DUMP, pInfo))
    {
        ...
        if (pInfo->FixupNum)
        {
            ...
            for (n = 0; n < pInfo->FixupNum; n++)
            {
                ...
                {
                    instrLen = GetDisasm().Disassemble(pInfo->Dump + pos, (__int64)curAdr, &DisInfo, disLine);
                    if (!instrLen)
                    {
                        //return;
...
void __fastcall TFKBViewer_11011981::btnOkClick(TObject *Sender)
{
    ...
    if (KnowledgeBase.GetProcInfo(CurrIdx, INFO_DUMP | INFO_ARGS, pInfo))
    {
        ...
        StrapProc(ap, CurrIdx, pInfo, false, EstimateProcSize(CurProcAdr));
                            ...
                            if (Idx != -1)
                            {
                                ...
                                if (!KnowledgeBase.IsUsedProc(Idx))
                                {
                                    if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo))
                                        StrapProc(ap, Idx, pInfo, true, pInfo->DumpSz);
                                }
                            }
                            else
                            {
                                ...
                                if (Idx != -1)
                                {
                                    ...
                                    {
                                        if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo))
                                            StrapProc(ap, Idx, pInfo, false, EstimateProcSize(adr));
                                    }
                                }
                            }
                        }
        ...
        //FMain_11011981->ShowUnitItems(GetUnit(CurUnitAdr), FMain_11011981->lbUnitItems->TopIndex, FMain_11011981->lbUnitItems->ItemIndex);
        ::SendMessage(Application->MainForm->Handle, WM_SHOWCODE, 3, 1); //todo const 3, 1 -> defines
    }
    Close();
}
//---------------------------------------------------------------------------

...
__fastcall TResourceInfo::~TResourceInfo()
{
    ...
    delete FormList; FormList = 0;
    delete Aliases; Aliases = 0;

}
//---------------------------------------------------------------------------
int __fastcall TResourceInfo::AnalyzeSection(TDfm* Dfm, TStringList* FormText, int From, PComponentInfo CInfo, int objAlign)
{
    int     n, pos, align;
    ...
    for (n = From; n < FormText->Count; n++)
    {
        ...
        if (line.Pos("object ") == 1 || inherited)
        {
            ...
            {
                componentName = "_" + Val2Str4(Counter) + "_"; Counter++;
            ...
            if (!HasAlias(cInfo->ClassName))
                AddAlias(cInfo->ClassName);
...
bool __stdcall EnumResNameProcedure(int hModule, char* Type, char* Name, long Param)
{
    ...
    BYTE                signature[4];
    TResourceInfo*      pThis = (TResourceInfo*)Param;
    ...
    if (Type == RT_RCDATA || Type == RT_BITMAP)
    {
        ...
        if (Type == RT_RCDATA)
        {
            ...
            res = true;
            if (pThis->GethFormPlugin())
            {
                fnGetDstSize = (int (__stdcall*)(BYTE*, int))GetProcAddress(pThis->GethFormPlugin(), "GetDstSize");
                fnDecrypt = (boolean (__stdcall*)(BYTE*, int, BYTE*, int))GetProcAddress(pThis->GethFormPlugin(), "Decrypt");
            ...
            if (res)
            {
                ...
                if (signature[0] == 'T' && signature[1] == 'P' && signature[2] == 'F' && (signature[3] >= '0' && signature[3] <= '7'))
                {
                    if (signature[3] == '0')
                    {
                        ...
                        pThis->GetFormAsText(dfm, formText);
                        ...
                        if (line.Pos("object ") == 1 || inherited)
                        {
                            ...
                            pThis->AnalyzeSection(dfm, formText, 1, 0, 1);
                        }
                        delete formText;

                        pThis->AddDfm(dfm);
                    }
                    else if (!idr.ResInfo()->GetIsCitadel())
                    {
                        ShowMessage("Citadel for Delphi detected, forms cannot be loaded");
                        idr.ResInfo()->SetIsCitadel(true);
                    }
                }
                else
                {
...
void __fastcall TResourceInfo::AddDfm(TDfm* dfm)
{
    FormList->Add((void*)dfm);
}
void __fastcall TResourceInfo::AddAlias(String name)
{
    Aliases->Add(name);
}
bool __fastcall TResourceInfo::HasAlias(String name) const
{
    return (Aliases->IndexOf(name) != -1);
}
TDfm* __fastcall TResourceInfo::GetDfm(int idx) const
{
    assert(idx >=0 && idx < FormList->Count);

    TDfm *dfm = (TDfm*)FormList->Items[idx];
    return dfm;
}
//---------------------------------------------------------------------------
bool __fastcall TResourceInfo::EnumResources(String AppDir, String FileName)
{
	citadel = false;
    hFormPlugin = LoadLibrary((AppDir + "Plugins\\" + FormPluginName).c_str());
    ...
    for (int n = 0; n < cnt; n++)
    {
        ...
        if (dfm->Open == 2) //still opened
        {
            if (dfm->Form && dfm->Form->Visible)
            {
                ...
                //notify main window that we have to show the dfm window
                ::SendMessage(Application->MainForm->Handle, WM_DFMCLOSED, (unsigned int)dfm, 0);
            }
        }
    }
}

void __fastcall TResourceInfo::Clean()
{
    const int cnt = GetDfmCount();
    for (int n = 0; n < cnt; n++)
    {
...
void __fastcall TResourceInfo::FreeFormPlugin()
{
    FormPluginName = "";
...
void __fastcall TResourceInfo::SetAliasValue(String key, String val)
{
    Aliases->Values[key] = val;
}
String __fastcall TResourceInfo::GetAlias(int idx)
{
    assert(idx >=0 && idx < Aliases->Count);

    return Aliases->Strings[idx];
}
//---------------------------------------------------------------------------
...
void __fastcall TResourceInfo::GetEventsList(String FormName, TList* Lst)
{
    ...
    if (dfm->Events->Count)
    {
        ...
        for (int k = 0; k < evCnt; k++)
        {
            ...
            PMethodRec recM = GetMethodInfo(recN, methodName);
    ...
    for (int m = 0; m < compCnt; m++)
    {
        ...
        for (int k = 0; k < evCnt; k++)
        {
            ...
            //Find method address
            PMethodRec recM = GetMethodInfo(recN1, methodName);
            item->Adr = (recM) ? recM->address : 0;
            Lst->Add(item);
        }
    }
...
void __fastcall IdrDfmForm::miClick(TObject* Sender)
{
    ...
    if (mi->Tag)
    {
        ::SendMessage(Application->MainForm->Handle, WM_SHOWCODE, 0,
            (long)new ShowCodeData(mi->Tag, 0, -1, -1));
    }
}
//---------------------------------------------------------------------------
...
void __fastcall IdrDfmForm::ShowMyPopupMenu(String FormName, String ControlName, bool show)
{
    int         n;
    TMenuItem   *mi;
    TDfm* dfm = idr.ResInfo()->GetFormIdx(FormName, &n);
    if (!dfm) return;
    
    DWORD classAdr = GetClassAdr(dfm->ClassName);
    PInfoRec recN = idr.GetInfos(classAdr);
    ...
    if (SameText(dfm->Name, ControlName))
    {
        ...
        if (dfm->Flags & FF_INHERITED)
        {
            TDfm* parentDfm = idr.ResInfo()->GetParentDfm(dfm);
                ...
                for (int k = 0; k < ev->Count; k++)
                {
                    ...
                    if (SameText(eInfo->EventName, "Action"))
                    {
                        ...
                        if (dotpos)
                        {
                            ...
                            for (int j = 0; j < idr.ResInfo()->GetDfmCount(); j++)
                            {
                                TDfm* dfm1 = idr.ResInfo()->GetDfm(j);
                                if (SameText(dfm1->Name, moduleName))
                                {
                                    classAdr = GetClassAdr(dfm1->ClassName);
                                    recN = idr.GetInfos(classAdr);
    ...
    if (Adr && IsValidImageAdr(Adr))
    {
        ...
        if (recN)
        {
        	if (recN->kind == ikVMT)
            {
                ...
                ::SendMessage(Application->MainForm->Handle, WM_SHOWCLASSVIEWER, 0, Adr);
            }
            else
            {
    			//FMain_11011981->ShowCode(Adr, 0, -1, -1);
                ::SendMessage(Application->MainForm->Handle, WM_SHOWCODE, 0,
                    (long)new ShowCodeData(Adr, 0, -1, -1));
            }
        }
    }
    else
        ShowMessage("Handler not found, Address: "+Adr);
}
//---------------------------------------------------------------------------
void __fastcall IdrDfmForm::SetupControlHint(String FormName, TControl* Control, String InitHint)
{
    for (int n = 0; n < idr.ResInfo()->GetDfmCount(); n++)
    {
                ...
                if (SameText(cInfo->Name, mi->Name))
                {
                    ...
                    for (int k = 0; k < ev->Count; k++)
                    {
                        ...
                        if (SameText(eInfo->EventName, "OnClick"))
                        {
                            methodName = dfm->ClassName + "." + eInfo->ProcName;
                            //Ищем адрес соответствующего метода
                            recM = GetMethodInfo(recN, methodName);
                            mi->Tag = (recM) ? recM->address : 0;
                            mi->OnClick = miPopupClick;
                            //mi->Enabled = true;
                            continue;
...
void __fastcall IdrDfmForm::MyFormClose(TObject *Sender, TCloseAction &Action)
{
    ...
    ::SendMessage(Application->MainForm->Handle, WM_DFMCLOSED, 0, 0);
...
void __fastcall IdrDfmLoader::FindComponentClass(TReader* Reader, String ClassName, TMetaClass* &ComponentClass)
{
    ...
    catch (Exception& e)
    {
        ...
        String alias = idr.ResInfo()->GetAlias(ClassName);
        for (int n = 0;;n++)
        {
            if (!RegClasses[n].RegClass) break;
...
void __fastcall IdrDfmLoader::SetComponentName(TReader* Reader, TComponent* Component, String &Name)
{
    ...
    if (rc)
    {
        ...
        try
        {
            ...
            idr.ResInfo()->GetAlias(lastClassAliasName);
        }
        catch(...)
        {
        }
